...
def usage():
    return '''
constructs adjacency_graphs.py from QWERTY and DVORAK keyboard layouts

usage:
%s zxcvbn/adjacency_graphs.py
''' % sys.argv[0]
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1], 'w') as f:
        ...
        for graph_name, args in [('qwerty', (qwerty, True)),
            ...
            lines.append('"%s": %s' % (graph_name, json.dumps(graph, sort_keys=True).replace('null', 'None')))
        f.write(',\n    '.join(lines))
        f.write('\n}')
    sys.exit(0)
